...
const Profiles = require("../../users/model/Profile");
const verifyAuth = require("../../../core/middleware/verifyAuth");
...
router.put("/updateblog/:id", verifyAuth, async (req, res) => {
  try {
    ...
    const blog = await Blogs.findByIdAndUpdate(id, updateData, { new: true });
...
router.patch("/blog/like/:id", verifyAuth, async (req, res) => {
  try {
    ...
    const updatedBlog = await Blogs.findByIdAndUpdate(id, update, {
      new: true,
    });
...
router.get("/blogs/related/:id", async (req, res) => {
  try {
    ...
    if (tagNames.length === 0) {
      return res
        .status(200)
        .send({ message: "Related blogs found", blogs: [] });
    }
...
router.post("/blog/draft", verifyAuth, async (req, res) => {
  try {
    const { title, description, author, paragraphs, blogImage, tags } =
      req.body;
    if (!author) {
